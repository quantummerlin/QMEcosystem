#!/usr/bin/env python3
"""
Rebuild Rising Sign tool with actual astronomical calculations
...
<head>
    ...
    <title>Rising Sign Calculator & Reading | Quantum Merlin</title>
    <meta name="description" content="Calculate your Rising Sign from birth data and discover your outer personality through deep astrological wisdom.">
...
<body>
    <div class="container">
        ...
        <div class="tool-card">
            ...
            <form id="calculator-form" onsubmit="calculateRisingSign(event)">
                ...
                <button type="submit">ðŸ”® Calculate My Rising Sign & Reading</button>
            </form>
    ...
    <script>
        ...
        async function calculateRisingSign(event) {
            ...
            submitBtn.textContent = 'ðŸ”® Calculating astronomical positions...';
            ...
            try {
                ...
                const risingSign = calculateRisingPosition(birthDate, birthTime, coords);
                
                // Show result
                showResult(name, risingSign);
                saveUserData();
        ...
        function calculateRisingPosition(dateStr, timeStr, coords) {
            // Combine date and time into Date object
            const datetime = new Date(`${dateStr}T${timeStr}:00`);
            
            // Use Astronomy Engine to calculate Moon's ecliptic longitude
            const moonPos = Astronomy.Horizon(Astronomy.MakeTime(datetime));
            
            // Get Moon's equatorial coordinates
            const moonEqu = Astronomy.Horizon, Astronomy.MakeTime(datetime), null, true, false);
        ...
        function showResult(name, risingSign) {
            const interpretation = interpretations[risingSign];
            const personalizedReading = interpretation.replace(/{preferredName}/g, name);
            const paragraphs = personalizedReading.split('\\n\\n').map(p => `<p>${p}</p>`).join('');

            document.getElementById('result-title').textContent = `Moon in ${risingSign}: ${name}`;
...
print("ðŸ”§ Generating new Rising Sign Calculator with astronomy-engine...")

# Read existing interpretations from rising-sign-reading.html
try:
    with open('rising-sign-reading.html', 'r', encoding='utf-8') as f:
        content = f.read()
        ...
        if match:
            ...
            print("âš ï¸  Could not find interpretations in existing file, using defaults")
...
# Write new file
with open('rising-sign-reading-new.html', 'w', encoding='utf-8') as f:
    f.write(final_html)
