...
function downloadReading() {
    ...
    const fileName = `${userData.name || 'soul'}-reading-${new Date().toISOString().split('T')[0]}.html`;
...
<head>
    ...
    <title>${userData.name}'s Soul Blueprint</title>
    <style>
...
<body>
    <div class="container">
        <header>
            <h1>${userData.name}'s Soul Blueprint</h1>
...
async function shareReading() {
    const userData = JSON.parse(localStorage.getItem('lastReading') || '{}');
    const shareData = {
        title: `${userData.name}'s Soul Blueprint`,
        text: `Check out ${userData.name}'s complete soul reading with 65+ personalized insights!`,
